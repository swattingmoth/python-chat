...
of environment and OpenAI client construction).
"""
...
@patch("python_chat.api.OpenAI")
@patch("python_chat.api.os.getenv")
@patch("python_chat.api.load_dotenv")
def test_init_api_loads_env_and_constructs_openai_client(
    mock_load: MagicMock, mock_getenv: MagicMock, mock_openai: MagicMock
) -> None:
    """init_api calls load_dotenv, reads the named env var, and returns configured client."""
    ...
    mock_openai.assert_called_once_with(
        api_key="sk-test-123", base_url="https://api.x.ai/v1"
    )
...
def test_init_api_allows_none_api_key_for_testing(
    ...
    """Handles missing key gracefully (OpenAI client accepts None in some modes)."""
    ...
    mock_openai.assert_called_once_with(api_key=None, base_url="https://example.com")
    assert result is fake_client
